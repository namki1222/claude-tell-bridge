...
**3. 첫 핑퐁 (P2P)** — `api` 패널의 Claude에게 채팅으로 부탁한다:

```
tell demo web "핑퐁 테스트. 받은 키로 tell -r 로 '퐁 정상' 한 줄만 응답해줘" 실행해줘
```

몇 초 뒤 `api` 패널 입력창에 이렇게 돌아오면 성공이다:
...
| `tell rm <세션>` | 세션 정리 — tmux 종료 + 설정에서 제거 (각각 y/N 확인) |
